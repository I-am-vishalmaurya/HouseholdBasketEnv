...
See [`docs/plan_for_finale.md`](../docs/plan_for_finale.md) for the full
design.
...
- **Tasks:** 3 difficulty tiers (1 member / 3 steps, 2 members / 5 steps,
  3 members / 7 steps + adversarial set).
...
- **Policy path:** Qwen2.5-3B-Instruct fine-tuning with **Unsloth + GRPO**
  is provided in the notebooks.
- **Base data:** packaged-food catalog in `data/products.json`, augmented
  in-memory with `meal_type`, `price_inr`, `protein_g`, and `fiber_g`.
...
│   └── results/                  # reward-hack outputs and notebook results
...
Browse `http://localhost:8000/docs` for the auto-generated API surface.
...
```bash
uv run python -m household_basket_env.server.seed_verifier --task 1
uv run python -m household_basket_env.server.seed_verifier --task 2
uv run python -m household_basket_env.server.seed_verifier --task 3
```
...
| #   | Notebook                         | Phase        | Output when run locally / in Colab                                                          |
| --- | -------------------------------- | ------------ | ------------------------------------------------------------------------------------------- |
...
guard against trivial reward gaming.
...
- Phase 1–4 — env, client, server, manifest, Dockerfile, tests (58/58 green)
- Phase 1.5 — verified seeds for all 3 tiers
- Phase 4.5 — baseline eval notebook
- Phase 6/7/8 — training notebook (main + Ablation A + Ablation B)
- Phase 9/10 — eval + plots notebook
- Phase 11 — deploy script + this README
